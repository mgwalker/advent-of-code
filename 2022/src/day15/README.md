...
<p>In this example, in the row where <code>y=10</code>, there are <code><strong>26</strong></code> positions where a beacon cannot be present.</p>
...
<p>To isolate the distress beacon's signal, you need to determine its <strong>tuning frequency</strong>, which can be found by multiplying its <code>x</code> coordinate by <code>4000000</code> and then adding its <code>y</code> coordinate.</p>
<p>In the example above, the search space is smaller: instead, the <code>x</code> and <code>y</code> coordinates can each be at most <code>20</code>. With this reduced search area, there is only a single position that could have a beacon: <code>x=14, y=11</code>. The tuning frequency for this distress beacon is <code><strong>56000011</strong></code>.</p>
<p>Find the only possible position for the distress beacon. <strong>What is its tuning frequency?</strong></p>
</article>
