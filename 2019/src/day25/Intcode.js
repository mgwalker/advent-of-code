  ...
  put(message) {
    if (typeof message === "string") {
      message
        ...
        .forEach((m) => this.#_messages.unshift(m));
      this.#_messages.unshift(10n);
    } else {
      this.#_messages.unshift(message);
    }

    if (this.#_awaiting) {
      this.#_awaiting(this.#_messages.pop());
      this.#_awaiting = null;
    }
  }
...
export default async (ops, inputQueue, onOutput) => {
  ...
  while (true) {
    ...
    switch (op) {
      ...
      case 3n: // input
        {
          const input = await inputQueue.get();
          if (typeof input === "string") {
            return;
          }
          memory.set(ao, BigInt(input));
          ip += 2n;
        }
        break;
